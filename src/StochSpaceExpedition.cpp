...
string commands[20] = {"stats", "Groche", "FoodSpaceship", "FuelSpaceship", "Carrot", "MiniCarrotFarmRateUpgrader", "MiniCarrotFarmCapacityUpgrader", "FusionReactorRateUpgrader", "box", "cabinet", "safe", "replant", "harvest", "Foo", "reactor", "activate", "collect", "Nominee", "operate", "harvest"}; // All possible commands
...
int purpose(string input) { // Returns a string, assuming input is valid
  if (input == "stats") return 0; // 0 for stats
  else if (index(input) != 999) return 1; // 1 for move
  else return 2; // 2 for interact
}

int index(string input) { // Returns location or command index from respective arrays
  for (int i = 0; i < 7; i++) {
    if (input == locations[i]) return i;
  }
  for (int i = 0; i < 20; i++) {
    if (input == commands[i]) return i;
  }
  return 999;
}
...
void update() { // Makes available[] correct for a given current location
  available[location] = false; // Can't move to current location
...
bool valid(string input) { // Checks if input is valid based on if it is a valid location or command
  ...
  for (int i = 0; i < 20; i++) if (input == commands[i]) return true;
  return false;
}
